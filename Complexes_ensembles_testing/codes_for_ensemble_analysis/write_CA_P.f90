...
10  close(11)
	...
	inquire(file=trim(pdb_id)//"_"//trim(chain_to_read)//"_CA_P.pdb",exist=exist)
	if (exist) then
		open(14,file = trim(pdb_id)//"_"//trim(chain_to_read)//"_CA_P.pdb",status = 'old')
...
200 format(a4,i7,2x,a3,a1,a3,1x,a1,i4,4x,3f8.3)
	...
	open(15,file = trim(pdb_id)//"_"//trim(chain_to_read)//"_CA_P.pdb",status = 'new')
	count = 0
	do i = 1, num_chains
		...
		do n = 1, num_atoms
			...
			if (atom_type(1:2) == 'CA' .or. trim(atom_type) == 'P') then
